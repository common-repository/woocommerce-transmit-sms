...
class WC_BurstSMS {
    public function __construct() {
		...
		setcookie("BSMSorderReciveSentNotification", 0, time() + 3600); 
    ...
    public function checkedSentNotification(){
     ...
     public function sentNotification($order_id){
        ...
        //get comment
        global $wpdb;
        $resultsComent = $wpdb->get_results( 'SELECT * FROM wp_comments WHERE comment_post_ID ="'.$order_id .'" 
                             AND comment_content NOT LIKE  "Order status changed from%"  order by comment_date desc limit 1 ', OBJECT );
        $commentContent = '';
        if(is_array($resultsComent) && sizeof($resultsComent) > 0){
            foreach($resultsComent  as $key=>$val){
                 $commentContent = trim($val->comment_content);
            }
        }
        ...
        if((int)$sendChangeStatusNotif == 1 ){ 
            ...
            $this->sendSMS($order,$order->status.'Custom',  false,$product_name,$product_qty,$commentContent);
    ...
    public function sendSMS($order,$type,  $toAdmin = false,$product_name = null,$product_qty=null,$commentContent=null){ //type = Custom
        ...
        $arryReplaceShortcode= array($order->get_order_number(),date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) ), strip_tags($order->get_formatted_order_total()),
                                    $order->payment_method_title,$order->billing_first_name,$order->billing_last_name,$order->billing_phone,$order->billing_email,$order->billing_company,$order->billing_address_1,$order->billing_address_2,
                                        ...
                                        $product_name,$product_qty,$commentContent);
    ...
    public function sentNotificationforOrderRecive(){
        ...
        $shippingPhoneNumber =  get_post_meta( $order_id, 'shipping_phone', true );    
		if(isset($_COOKIE['BSMSorderReciveSentNotification']) && (int)$_COOKIE['BSMSorderReciveSentNotification'] === 0) {
        //if(!isset($_SESSION['BSMSorderReciveSentNotification'])){
           //add to list
            ...
            global $wpdb;
            $resultsComent = $wpdb->get_results( 'SELECT * FROM wp_comments WHERE comment_post_ID ="'.$order_id .'" order by comment_date desc limit 1 ', OBJECT );
            $commentContent = '';
            if(is_array($resultsComent) && sizeof($resultsComent) > 0){
                foreach($resultsComent  as $key=>$val){
                     $commentContent = trim($val->comment_content);
            ...
            $this->sendSMS($order,'receivedCustom',true,$product_name,$product_qty,$commentContent);
			...
			setcookie("BSMSorderReciveSentNotification", 1, time() + 3600); 
			/*
